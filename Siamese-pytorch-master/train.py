...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------#
    # ----------------------------------------------------#
    #   gpu_id      指定使用哪块GPU进行训练
    #               如果为None，则自动选择可用的GPU
    #               如果为0、1、2等数字，则使用指定的GPU
    #               例如：gpu_id = 0 表示使用第0块GPU
    
    # ----------------------------------------------------#
    gpu_id = 0  # 指定使用的GPU ID，可以改为None、0、1、2等（使用GPU 3，最后一块空闲的GPU）
    ...
    if Cuda:
        if gpu_id is not None:
            ...
            print(f"✅ 检测到CUDA支持，将使用GPU {gpu_id}训练")
            print(f"GPU名称: {torch.cuda.get_device_name(gpu_id)}")
        else:
            print("✅ 检测到CUDA支持，将使用默认GPU训练")
    else:
        print("⚠️ 未检测到CUDA支持，将使用CPU训练")
    # ---------------------------------------------------------------------#
    ...
    #   数据集存放的路径
    # ----------------------------------------------------#
    dataset_path = "D:\data2\dataset1211\chewei"  # 使用output1文件夹下的车牌号数据
    # ----------------------------------------------------#
    ...
    # ------------------------------------------------------------------#
    save_dir = 'D:\project\data_chuli\demo\demo\Siamese-pytorch-master\logs\weibu\\1211'  # 训练日志和权重保存路径
    # ------------------------------------------------------------------#
    ...
    if Cuda:
        if distributed:
            ...
            # 单GPU模式：使用指定的GPU
            if gpu_id is not None:
                model_train = torch.nn.DataParallel(model, device_ids=[gpu_id])
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, Epoch):
            ...
            # 在非分布式模式下，local_rank用于控制日志，device_id用于数据移动
            actual_device_id = gpu_id if (not distributed and gpu_id is not None) else None
            fit_one_epoch(model_train, model, loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen,
                          gen_val, Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank, actual_device_id)
